...
ras,decs = loadtxt(args.coord_list,skiprows=1,unpack=True)
...
for ra,dec in zip(ras,decs):
    ...
    cmd = 'wget "http://vo.astron.nl/getproduct/tgssadr/fits/TGSSADR_%s_5x5.MOSAIC.FITS?sdec=%.2f&dec=%.10f&ra=%.10f&sra=%.2f" -O RA%.2fDEC%.2f_TGSS.fits' %(pointing,im_size,dec,ra,im_size,ra,dec)
    call(cmd,shell=True)
